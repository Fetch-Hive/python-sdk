...
class InvokeHiveAgentResponse(BaseModel):
    """
    Hive Agent run accepted response.
    """ # noqa: E501
    run_id: Optional[StrictStr] = Field(default=None, description="Identifier for the queued Hive Agent run.")
    request_id: Optional[StrictStr] = Field(default=None, description="Unique identifier for this invocation.")
    status: Optional[StrictStr] = Field(default=None, description="Initial run status. Usually `pending`.")
    webhook_secret: Optional[StrictStr] = Field(default=None, description="Secret used to verify the signed callback.")
    __properties: ClassVar[List[str]] = ["run_id", "request_id", "status", "webhook_secret"]

...
class InvokeWorkflowResponse(BaseModel):
    ...
    run_id: Optional[StrictStr] = Field(default=None, description="Workflow run identifier.")
    status: Optional[StrictStr] = Field(default=None, description="Final status of the workflow run.")
    output: Optional[StrictStr] = Field(default=None, description="The workflow's final output value. May contain serialised JSON; parse client-side as needed. ")
    error: Optional[StrictStr] = Field(default=None, description="Error message when `status` is `failed`.")
    __properties: ClassVar[List[str]] = ["request_id", "run_id", "status", "output", "error"]

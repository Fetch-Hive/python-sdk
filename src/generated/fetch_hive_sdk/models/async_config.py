...
class AsyncConfig(BaseModel):
    ...
    AsyncConfig
    """ # noqa: E501
    enabled: StrictBool = Field(description="When `true`, the endpoint returns immediately with a `run_id` rather than waiting for the workflow to complete. ")
    callback_url: Optional[StrictStr] = Field(default=None, description="Optional HTTPS URL that receives a POST with the workflow result when execution finishes (async mode only). ")
    __properties: ClassVar[List[str]] = ["enabled", "callback_url"]
